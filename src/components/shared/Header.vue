<script lang="ts">
import IconLogo from '../icons/IconLogo.vue';

import { useScreenStore } from '../../stores/screen';
import { useThemeStore } from  '../../stores/theme';
 import  { storeToRefs } from 'pinia';

export default {
      setup() {
            const store = useScreenStore();
            const  themeStore  = useThemeStore();

            const { getDeviceType } = storeToRefs(store);
            const { getThemeMode } = storeToRefs(themeStore);
            return { getThemeMode, getDeviceType, themeStore }
      },

    components: {IconLogo},
    data() {
        return {
            notLogin:  true,
            poopupActive: false
        }
    },
    methods: {
      togglePopup() {
        this.poopupActive = !this.poopupActive;
      }
    }

}
</script>

<template>
    <div class="header-container">
        <div class="logo-container">
            <IconLogo />
        </div>
        <div class="header-controls">
            <div class="toggle-btn-wrapper">
                <button class="toggle-btn" >
                <svg v-if="getThemeMode != 'dark'" width="20" height="20" xmlns="http://www.w3.org/2000/svg" @click="() => themeStore.themeMode = 'dark'">
                <path
                  d="M19.502 11.342a.703.703 0 00-.588.128 7.499 7.499 0 01-2.275 1.33 7.123 7.123 0 01-2.581.46A7.516 7.516 0 018.74 11.06a7.516 7.516 0 01-2.198-5.316c0-.87.153-1.713.41-2.48.28-.817.69-1.559 1.226-2.197a.652.652 0 00-.102-.92.703.703 0 00-.588-.128C5.316.607 3.425 1.91 2.07 3.649A10.082 10.082 0 000 9.783C0 12.57 1.125 15.1 2.965 16.94a10.04 10.04 0 007.156 2.965c2.352 0 4.524-.818 6.262-2.173a10.078 10.078 0 003.579-5.597.62.62 0 00-.46-.793z"
                  fill="#7E88C3"
                  fillRule="nonzero"
                />

              </svg>
            <svg  v-else width="20" height="20" xmlns="http://www.w3.org/2000/svg" @click="() => themeStore.themeMode = 'light'">
                <path
                  d="M9.817 16.18a.96.96 0 01.953.848l.007.112v1.535a.96.96 0 01-1.913.112l-.006-.112V17.14c0-.53.43-.96.96-.96zm-4.5-1.863c.347.346.373.89.08 1.266l-.08.09-1.085 1.087a.96.96 0 01-1.437-1.267l.08-.09 1.086-1.086a.959.959 0 011.357 0zm10.356 0l1.086 1.086a.959.959 0 11-1.357 1.357l-1.085-1.086a.959.959 0 111.356-1.357zM9.817 4.9a4.924 4.924 0 014.918 4.918 4.924 4.924 0 01-4.918 4.918A4.924 4.924 0 014.9 9.818 4.924 4.924 0 019.817 4.9zm8.858 3.958a.96.96 0 110 1.919H17.14a.96.96 0 110-1.92h1.535zm-16.18 0a.96.96 0 01.112 1.912l-.112.007H.96a.96.96 0 01-.112-1.913l.112-.006h1.534zm14.264-5.983a.96.96 0 010 1.357l-1.086 1.086a.96.96 0 11-1.356-1.357l1.085-1.086a.96.96 0 011.357 0zm-12.617-.08l.09.08 1.086 1.086A.96.96 0 014.05 5.398l-.09-.08-1.086-1.086a.959.959 0 011.267-1.436zM9.817 0c.53 0 .96.43.96.96v1.535a.96.96 0 01-1.92 0V.96c0-.53.43-.96.96-.96z"
                  fill="#858BB2"
                  fillRule="nonzero"
                />
              </svg>
                </button>
            </div>
            <div class="avatar-wrapper">
                <img
                @click="togglePopup"
                src="../../assets/images/image-avatar.jpg"
                alt="Avatar"
              />
              <div class="popup popup-dark" v-if="poopupActive">
                  <li>Sign Out</li>
              </div>
            </div>
        </div>
    </div>
</template>

<style lang="scss" scoped>
@import '../../scss/_mixins.scss';
@import '../../scss/_variables.scss';

.header-container {
  background-color: $ebony;
  height: 4.5rem;
  width: 100%;
  display: flex;
  align-items: center;
  justify-content: space-between;
  position: fixed;
  z-index: 2;
  top: 0;

  @media (min-width: $tablet) {
  }

  @media (min-width: $desktop) {
    height: 100vh;
    position: fixed;
    left: 0;
    width: 6.4375rem;
    border-radius: 0px 20px 20px 0px;
    flex-direction: column;
    z-index: 3;
  }
}

.logo-container {
  background-color: $purple;
  width: 4.5rem;
  height: 4.5rem;
  display: flex;
  align-items: center;
  justify-content: center;
  border-radius: 0px 20px 20px 0px;
  position: relative;
  overflow: hidden;
  cursor: pointer;
  svg {
    z-index: 2;
  }

  &:after {
    content: "";
    width: 100%;
    height: 6.4375rem;
    position: absolute;
    background-color: $heliotrope;
    border-radius: 20px 0px 0px 20px;
    top: 50%;
    transition: 0.3s ease;
  }

  &:hover::after {
    top: 10%;
  }

  @media (min-width: $desktop) {
    width: 100%;
    height: 6.4375rem;
  }
}

.header-controls {
  display: flex;
  align-items: center;
  height: 100%;

  @media (min-width: $desktop) {
    flex-direction: column;
    width: 100%;
    height: auto;
  }
}

// togggle button styles
.toggle-btn  {
  background: none;
  border: none;
  padding: 0.25rem;

  &:hover {
    svg path {
      fill: $selago;
      transition: fill 0.1s ease-in-out;
    }
  }
}

.toggle-btn-wrapper {
  border-right: 1px solid $darkAccent;
  display: flex;
  align-items: center;
  justify-content: center;
  height: 100%;

  @media (min-width: $desktop) {
    border-right: none;
    border-bottom: 1px solid $darkAccent;
  }
}

// Avatar  
.avatar-wrapper {
      cursor: pointer;
  &:hover {
    img {
      border: 3px solid $purple;
    }
  }
}

.avatar-wrapper  img {
  width: 2rem;
  height: 2rem;
  border-radius: 50%;
}

// POP UP  
.popup {
  min-width: 180px;
  position: absolute;
  background-color: white;

  right: 15%;
  top: 80%;
  z-index: 40;
  border-radius: 0.5rem;
  padding: 1.5rem;
  text-align: center;
  ul {
    list-style: none;

    li:hover {
      color: $purple;
      text-decoration: underline;
      cursor: pointer;
    }
  }
  color: $vulcan;
  @include normalText;
  font-weight: bold;

  @media (min-width: $desktop) {
    top: 0%;
    left: 105%;
  }
}

.popup-dark  {
  background-color: $mirage;
  color: white;
}

// Avatar and Toggle Btn
.avatar-wrapper, .toggle-btn-wrapper {
  min-width: 5rem;
  display: flex;
  align-items: center;
  position: relative;
  justify-content: center;
  height: 100%;
  @media (min-width: $tablet) {
    width: 6rem;
  }

  @media (min-width: $desktop) {
    width: 100%;
    height: 5.5rem;
  }
}
</style>